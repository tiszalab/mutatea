...
## script for aligning reads, merging BAM files by only month.year to compare wastewater results directly against clinical results
...
## set directories
BASE_DIR="/gpfs1/projects/Tisza_Lab/crm_flu_mutatome/${VARIANT}_align/MY_only"
...
while IFS=',' read -r -a cols; do
    ...
    ## want each list file to be named by the month it is being grouped by
    MERGE_KEY="${DATE}"
...
## merge BAMs for each month combination
