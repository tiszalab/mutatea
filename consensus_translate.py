#!/usr/bin/env python3
"""
Translate consensus FASTA nucleotide sequences to amino acid sequences
using CDS annotations from per-subtype GFF files.

Input:  test_runs/consensus/{subtype}.{source}.{year}.fasta
Output: test_runs/consensus/amino_acids/{subtype}.{source}.{year}.aa.fasta

Each output file contains one AA record per annotated CDS, with headers:
  >{subtype}.{source}.{year}.{gene_name}
"""

# executable: python3 /data/tisza/analyses/crm/mutatea/consensus_translate.py
...
CONSENSUS_DIR = Path("/data/tisza/analyses/crm/mutatea/test_runs/consensus")
OUT_DIR = CONSENSUS_DIR / "amino_acids"
...
for subtype in SUBTYPES:
    ...
    for fasta_path in sorted(CONSENSUS_DIR.glob(f"{subtype}.*.fasta")):
        stem = fasta_path.stem                        # e.g. H1N1.clinical.2022
        parts_split = stem.split(".")
        if len(parts_split) != 3:
            continue
        _, source, year = parts_split
        ...
        for cds_id, exons in cds_parts.items():
            ...
            if aa_seq is None:
                ...
                continue
            records.append(f">{subtype}.{source}.{year}.{gene_name}\n{aa_seq}")

        out_path = OUT_DIR / f"{stem}.aa.fasta"
